...
function getSchemaExample(schema, schemas = jsonData.components.schemas) {
    if (!schema || typeof schema !== 'object') return "string";

    // Resolve $ref references
    if (schema.$ref) {
        const refName = schema.$ref.split("/").pop();
        return getSchemaExample(schemas[refName], schemas);
    }
    ...
    switch (schema.type) {
        ...
        case 'array':
            if (schema.items) {
                return [getSchemaExample(schema.items, schemas)];
            }
            return ["string"];
        case 'object':
            let example = {};
            if (schema.properties) {
                Object.entries(schema.properties).forEach(([key, value]) => {
                    example[key] = getSchemaExample(value, schemas);
                });
            }
            return example;
        default:
    ...
    try {
        ...
        if (bodyElement && ['POST', 'PUT'].includes(method.toUpperCase())) {
            try {
                ...
                options.body = JSON.stringify(requestBody, null, 2);
            } catch (e) {
        ...
        // Display the generated request URL
        const requestDetails = `
            <h6>Request URL:</h6>
            <pre>${url.toString()}</pre>
            <h6>Request Body:</h6>
            <pre>${requestBody ? JSON.stringify(requestBody, null, 2) : 'N/A'}</pre>
        `;

        responseElement.innerHTML = requestDetails + '<p>Waiting for response...</p>';

        // Fetch API Response
        const response = await fetch(url, options);
        const data = await response.json();
        
        responseElement.innerHTML = requestDetails + `
            <h6>Response:</h6>
